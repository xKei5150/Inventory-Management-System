...
$(document).ready(function() {
	$.ajax({
			...
			success: function (data) {
				if(data.valid == valid){
					// console.log(data);
					$('#admin-account').append('<i class="fa-regular fa-user"></i> '+
								data.logged+
...
$('#qrModal').on('shown.bs.modal', function (e) {
	html5QrCode.start(
		...
		qrCodeMessage => {
			...
			let serialNumber = qrCodeMessage;
			item_profile_by_serial(serialNumber);
...
function item_profile_by_serial(serialNumber) {
	$('#modal-item-profile').modal('show');
	$.ajax({
		url: '../data/item_profile_by_serial.php',
		dataType: 'json',
		type: 'post',
		data: {
		...
		error: function(){
			alert('Error: item_profile_by_serial L136+');
		}
	});
}
